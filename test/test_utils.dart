...
/// Execute an external function with a temporarily created
/// database, containing a single table with colums designed
/// to test TIME WITH TIME ZONE and TIMESTAMP WITH TIME ZONE
/// Firebird data types.
/// Requires Firebird >= 4.0.
Future<void> withNewDb3(
  Future<void> Function(FbDb) testFunc, {
  FbOptions? options,
}) async {
  await withNewEmptyDb((db) async {
    ...
    try {
      ...
      await q.execute(
        sql:
            ...
            "values ("
            "   1, "
            "   '12:01:02.0304 Europe/London', "
            "   '2025-05-22 12:01:02.0304 Europe/Warsaw'"
            ")",
      );
      await q.execute(
        sql:
            ...
            "   2, "
            "   '12:01:02.0304 +00:00', "
            "   '2025-05-22 12:01:02.0304 +02:00'"
            ")",
      );
    } finally {
      await q.close();
