...
class IProvider extends IPluginBase {
  ...
  late FbInterface Function(
      ...
      int dpbLength,
      Pointer<Uint8> dpb) _attachDatabase;
  ...
  late FbInterface Function(
      ...
      Pointer<Utf8> fileName,
      int dpbLength,
      Pointer<Uint8> dpb) _createDatabase;
  ...
  late FbInterface Function(
      ...
      int spbLength,
      Pointer<Uint8> spb) _attachServiceManager;

  late void Function(
      FbInterface self, FbInterface status, int timeout, int reason) _shutdown;

  late void Function(
          FbInterface self, FbInterface status, FbInterface crytpCallback)
      _setDbCryptCallback;
  ...
  /// etc.).
  IAttachment createDatabase(IStatus status, String fileName,
      [int dpbLength = 0, Pointer<Uint8>? dpb]) {
    final fileNameUtf = fileName.toNativeUtf8(allocator: mem);
    try {
      dpb ??= nullptr;
      final res =
          _createDatabase(self, status.self, fileNameUtf, dpbLength, dpb);
      status.checkStatus();
      return IAttachment(res);
    } finally {
      mem.free(fileNameUtf);
